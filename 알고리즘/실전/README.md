...
| ----------------- | ------------------------- | ------------------------------ |
| `%`               | 시작 기호                     | `%d`, `%s` 등                   |
| `argument_index$` | 몇 번째 인자를 사용할지 (선택)        | `%2$d` → 두 번째 인자를 `int`로 출력    |
| `flags`           | 출력 형식 옵션 (좌측 정렬, 부호 등)    | `%-5s`, `%+d`, `%0d`           |
| `width`           | 최소 너비                     | `%10s` → 최소 10칸                |
| `.precision`      | 소수점 이하 자릿수 (float/double) | `%.2f` → 소수점 2자리               |
| `conversion`      | 형변환 기호                    | `d`, `f`, `s`, `c`, `x`, `b` 등 |

## 주요 변환 기호
| 변환 기호 | 설명                | 예시                                          |
| ----- | ----------------- | ------------------------------------------- |
| `%d`  | 정수 (decimal)      | `String.format("%d", 10)` → `"10"`          |
| `%f`  | 실수 (float/double) | `String.format("%.2f", 3.14159)` → `"3.14"` |
| `%s`  | 문자열               | `String.format("%s", "hi")` → `"hi"`        |
| `%c`  | 문자                | `String.format("%c", 'A')` → `"A"`          |
| `%b`  | boolean           | `String.format("%b", true)` → `"true"`      |
| `%x`  | 16진수 (소문자)        | `String.format("%x", 255)` → `"ff"`         |
| `%X`  | 16진수 (대문자)        | `String.format("%X", 255)` → `"FF"`         |
| `%%`  | `%` 자체            | `String.format("%%")` → `"%"`               |

## 예시
``` java
System.out.println(String.format("%-10s %04d %.2f", "item", 7, 3.1415));
// 출력: item       0007 3.14
```